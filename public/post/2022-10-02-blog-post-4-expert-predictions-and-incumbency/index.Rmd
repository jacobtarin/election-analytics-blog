...
For my model, I'm first going to predict the seat share of the incumbent president's party in the House using the incumbent president's party, the incumbent party in the House, and GDP growth in the last quarter before the election as an economic indicator. As I've discussed in previous weeks, observers of politics have noted that the president's party tends to lose seats during the midterm election, especially when the economy is doing poorly in the eyes of voters. I include the incumbent party in the house because I want to see whether voters are sensitive to the majority party of the House itself when voting.
...
model_seats_data <- seats_data %>% 
  ...
  mutate(H_incumbent_party_dummy = ifelse(H_incumbent_party == "D", 1, 0)) %>% # incumbent party in House dummy
...
pres_only_model <- lm(incumbent_pres_seatshare ~ president_party_dummy + midterm_dummy + GDP_growth_pct, data = model_seats_data)
# stargazer(pres_only_model, type = "text")

# Model 2: Fundamentals with House majority party
pres_plus_house_model <- lm(incumbent_pres_seatshare ~ president_party_dummy + H_incumbent_party_dummy + midterm_dummy + GDP_growth_pct, data = model_seats_data)
# stargazer(pres_plus_house_model, type = "text")
...
combo <- lm(incumbent_pres_seatshare ~ president_party_dummy + H_incumbent_party_dummy + midterm_dummy + incumbent_pres_poll_support + GDP_growth_pct, data = model_seats_data)
# stargazer(combo, type = "text")

# stargazer(pres_only_model, pres_plus_house_model, polls_only_model, combo, title = "Model Coefficients", 
#           align = TRUE, type = "latex", font.size = "small", header = FALSE)
```